...
try:
    with sync_playwright() as p:
        ...
        if not OFFLINE:
            ...
            page.get_by_role('link',name='继续实验设置 →').click()
            expect(page.get_by_role('heading',name='新品上市模拟',exact=True)).to_be_visible()
            expect(page.locator('.scheme-preview')).to_have_count(3)
            expect(page.locator('[data-page="results"]')).to_have_count(0)
            page.screenshot(path=str(QA/'10-product-workspace-desktop.png'),full_page=True)
            passed('Landing CTA opens the new workspace with configuration isolated from results')
            page.locator('.stepper a[href="#/setup/2"]').click()
            expect(page.get_by_role('heading',name='谁参与这个实验？')).to_be_visible()
            page.locator('.stepper a[href="#/setup/5"]').click()
            page.locator('[data-action="run"]').click()
            page.wait_for_url('**/#/results/*')
            expect(page.locator('[data-page="results"]')).to_have_count(1)
            expect(page.locator('[data-page="setup"]')).to_have_count(0)
            page.locator('[data-metric="conversion"]').click()
            expect(page.locator('[data-metric="conversion"]')).to_have_attribute('aria-pressed','true')
            page.get_by_role('link',name='查看消费者详情 →').click()
            expect(page.locator('[data-page="consumer"]')).to_have_count(1)
            expect(page.locator('[data-page="results"]')).to_have_count(0)
            passed('Real simulation opens independent results and then independent consumer evidence')
